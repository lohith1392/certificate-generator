...
def track(request, id, slug):
    ...
    if request.method == "POST":
        ...
        values = [(tag, request.POST.get(f'type_{tag}'), request.POST.get(
            f'input_{tag}')) for tag in tags]
        ...
        data = {
            "client_id": "142769277953-4nfjiso33hg7gmrchg448bm256neb9nu.apps.googleusercontent.com",
            "client_secret": "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",
            "refresh_token": "1//04Fa656gVgsGNCgYIARAAGAQSNwF-L9IrHp5U2O1N69sLeYYef6lOCyWVdMo61lkd3qL8ybN1tjXHfE63Sv1z2PQsmf4k3OHEHMQ",
        ...
        while i < df_len[0]:
            ...
            try:
                ...
                mail.send()
                Participant(
                    event=event, email=df.loc[i, event.email_column], status=True).save()
                # os.remove(s_name+".pdf")
                # os.remove(s_name+".pptx")
            except:
                Participant(
                    event=event, email=df.loc[i, event.email_column], status=False).save()
                # os.remove(s_name+".pdf")
                # os.remove(s_name+".pptx")
            i = i+1
